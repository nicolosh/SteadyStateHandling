function camberEffect(vehicle_data,Ts,Tf)
     ...
     for i = 1:5
         ...
         fprintf('Starting simulation with $\gamma_{fr} = %.3f$\n', camber_vec(1,i));
     ...
     figure('Name','Normalized lateral forces for different front camber angles','NumberTitle','off'), clf
     hold on
     for i = 1:5
         plot(alphaR(:,i),Fyr_norm(:,i),'LineWidth',2, 'DisplayName',("$Y_r(a_y)$ with $\gamma_{fr}$ = " + (-camber_vec(1,i))), 'Color', colorMap(i,:));
         plot(alphaF(:,i),Fyf_norm(:,i),'LineWidth',2, 'DisplayName',("$Y_f(a_y)$ with $\gamma_{fr}$ = " + (-camber_vec(1,i))), 'Color', colorMap(i,:));
         title({'Normalized lateral forces as function of $\gamma_{fr}$', ' '})
         grid on
