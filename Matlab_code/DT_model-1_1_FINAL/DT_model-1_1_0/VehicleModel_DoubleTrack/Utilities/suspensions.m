function suspensions(vehicle_data,Ts,Tf)
    
    
    epsilon_phi = vehicle_data.suspension.epsilon_phi;
    epsilon_phi_new = [0.3 epsilon_phi 0.5 0.6 0.65];    
    Ks_f_new = ((vehicle_data.suspension.KsT)*epsilon_phi_new);
    Ks_r_new = ((vehicle_data.suspension.KsT)*(1-epsilon_phi_new));
    time_sim = Tf;
    dt = Ts;
    
    for i = 1:5
        vehicle_data.front_suspension.Ks_f = Ks_f_new(1,i);
        vehicle_data.front_suspension.Ks_r = Ks_r_new(1,i);
    ...
    figure('Name','Normalized lateral forces ,suspensions','NumberTitle','off'), clf
    hold on
    for i = 1:5
        plot(alphaR_dt(:,i),Fyr_dt_norm(:,i),'LineWidth',2, 'DisplayName',("Yr(ay) with $\epsilon_\phi$ = " + epsilon_phi_new(1,i)), 'Color', colorMap(i,:));
        plot(alphaF_dt(:,i),Fyf_dt_norm(:,i),'LineWidth',2, 'DisplayName',("Yf(ay) with $\epsilon_\phi$ = " + epsilon_phi_new(1,i)), 'Color', colorMap(i,:));
    ...
    hold off
    colorMap = colormap(lines(5));
    figure('Name','Handling curves, suspensions','NumberTitle','off'), clf
    hold on
    for i = 1:5
        plot(Ay(10000:end,i)./g, Ackerman(10000:end)-Rho2(10000:end,i), 'LineWidth',2, 'DisplayName',("$\epsilon_\phi$ = " + epsilon_phi_new(1,i)), 'Color', colorMap(i,:));
